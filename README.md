...
학생–교수 이메일 상담 기록을 **Gmail에서 자동 수집**하고, **OpenAI API**(유료)를 이용해 상담 내용을 요약한 뒤  
**학교 상담 시스템에 업로드 가능한 엑셀 파일**로 변환하는 도구입니다.
...
- 모든 처리는 **교수 개인 PC 로컬 환경**에서만 이루어집니다.
- Gmail 접근은 **읽기 전용**(`gmail.readonly`) 권한만 사용합니다.
- 결과 엑셀 파일(`output.xlsx`)은 외부로 전송되지 않습니다.
