...
⚠️ API Key는 다시 확인할 수 없으므로 반드시 복사해 두세요. API Key는 개인 인증 정보이므로 외부에 공유하거나 Git에 커밋하지 마세요.
...
1. Gmail 검색창에서 필터링 규칙을 활용하여 상담 메일 후보를 검색  
   예: 교수님 -{교학팀} -{지원팀} -{총무팀}  -{운영팀} -{평가팀} -{비서실} -noreply
...
이는 다음과 같은 경우에 발생합니다.
- 학생이 학교 메일 계정이 아닌 다른 이메일로 상담 메일을 보냈고,
- 메일 본문에 학번 정보가 명시적으로 포함되어 있지 않은 경우

이 경우에는 **교수가 직접 학번을 확인하여 엑셀 파일에 수동으로 입력**해야 합니다.
